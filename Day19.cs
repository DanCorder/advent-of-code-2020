namespace advent_of_code_2020
{
    ...
    public class Day19
    {
        ...
        {
            var parts = ProblemInput.Split("\n\n");
            var rules = parts[0].SplitToLines().ToDictionary(x => Int32.Parse(x.Split(':')[0]), x => x.Split(':')[1].Trim());
            var messages = parts[1].SplitToLines().ToList();

            var currentRule = rules[0];

            var pr = $"^{buildRule(currentRule, rules)}$";
            var r = new Regex(pr);

            return messages.Count(m => r.IsMatch(m));
        }

        private static string buildRule(string rule, Dictionary<int, string> rules)
        {
            var ret = "";
            if (rule == "a" || rule == "b")
                return rule;
            ...
            {
                foreach(var n in part.Trim().Split(' '))
                {
                    ret += $"(?:{buildRule(rules[Int32.Parse(n)], rules)})";
                }
                ret += "|";
            }
        ...
        public static int SolveProblem2()
        {
            ...
            var pr = $"^{buildRule2(currentRule, rules)}$";
            var r = new Regex(pr);

            return messages.Count(m => r.IsMatch(m));
        }

        private static string buildRule2(string rule, Dictionary<int, string> rules)
        {
            var ret = "";
            if (rule == "a" || rule == "b")
                return rule;
            ...
            if (rule == "42 | 42 8")
                return $"(?:{buildRule2(rules[42], rules)})+";
            ...
            foreach(var part in parts)
            {
                foreach(var n in part.Trim().Split(' '))
                {
                    ret += $"(?:{buildRule2(rules[Int32.Parse(n)], rules)})";
                }
                ret += "|";
            }
            return ret.Substring(0, ret.Length - 1);
        ...
        private const string ProblemInput = @"66: 26 127 | 132 49
65: 99 132 | 100 26
40: 1 26 | 16 132
87: 16 26 | 49 132
124: 132 57 | 26 107
...
";
        private const string ProblemTestInput = @"0: 4 1 5
1: 2 3 | 3 2
2: 4 4 | 5 5
3: 4 5 | 5 4
4: a
