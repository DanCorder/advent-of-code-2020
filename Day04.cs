namespace advent_of_code_2020
{
    ...
    public class Day04
    {
        ...
        {
            var passports = ProblemInput.Split("\n\n")
                .Select(toFields);
        ...
        public static int SolveProblem2()
        {
            ...
            return passports
                .Count(isValidPassport2);
        }

        private static Dictionary<string, string> toFields(string passport)
        {
            var ret = passport.Split(new string[] { " ", "\n" }, StringSplitOptions.RemoveEmptyEntries)
            .ToDictionary<string, string, string>(v => v.Split(':')[0], v => v.Split(':')[1]);
        ...
        private static bool isValidPassport2(Dictionary<string, string> passport)
        {
            // byr (Birth Year)
            // iyr (Issue Year)
            // eyr (Expiration Year)
            ...
            // cid (Country ID)
            var keys = passport.Keys;
            var containsKeys = keys.Contains("byr") &&
                keys.Contains("iyr") &&
                keys.Contains("eyr") &&
                keys.Contains("hgt") &&
                keys.Contains("hcl") &&
            ...
            var rawHgt = passport["hgt"];
            if (rawHgt.EndsWith("cm"))
            {
                var hgt = int.Parse(rawHgt.Substring(0, rawHgt.Length - 2));
                if (hgt < 150 || hgt > 193)
                {
                    return false;
                }
            }
            else if (rawHgt.EndsWith("in"))
            {
                var hgt = int.Parse(rawHgt.Substring(0, rawHgt.Length - 2));
                if (hgt < 59 || hgt > 76)
                {
            ...
            var rawHcl = passport["hcl"];
            if (rawHcl.Length != 7 || !hairColourMatcher.IsMatch(rawHcl))
            {
                return false;
            }
            var rawEcl = passport["ecl"];
            if (!eyeColours.Contains(rawEcl))
            {
                return false;
            }
            var rawPid = passport["pid"];
            if (rawPid.Length != 9 || !passportIdMatcher.IsMatch(rawPid))
            {
                return false;
            }
        ...
        private static Regex hairColourMatcher = new Regex("#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]");
        private static Regex passportIdMatcher = new Regex("[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]");

        private static string[] eyeColours =
        {
            "amb",
            "blu",
            "brn", 
        ...
        private const string ProblemInput = @"byr:1985
