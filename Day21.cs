namespace advent_of_code_2020
{
    ...
    public class Day21
    {
        ...
        {
            var lines = ProblemInput.SplitToLines().Select(l => {
                    var parts = l.Split('(');
                    var allergens = parts[1].Replace("contains ", "").Replace(")", "").Replace(",", "").Trim();
                    return new Tuple<List<string>, List<string>>(
                        parts[0].Trim().Split(' ').ToList(),
                        allergens.Split(' ').ToList()
                    );
                })
                .ToList();

            var allIngredients = lines.SelectMany(l => l.Item1).Distinct().ToList();
            var allAllergens = lines.SelectMany(l => l.Item2).Distinct().ToList();

            var possibleIngredientsByAllergen = allAllergens.ToDictionary(
                a => a,
                a => lines.Aggregate((List<string>)null, (curr, l) => {
                    if (!l.Item2.Contains(a))
                        return curr;
                    if (curr == null)
                        return l.Item1;
                    return l.Item1.Intersect(curr).ToList();
                }));
            
            Dictionary<string, string> definiteIngredientsByAllergen = new Dictionary<string, string>();
            while(possibleIngredientsByAllergen.Any(kvp => kvp.Value.Count > 0))
            {
                var newDefiniteIngredientsByAllergen = possibleIngredientsByAllergen.Where(kvp => kvp.Value.Count == 1)
                    .ToDictionary(kvp => kvp.Key, kvp => kvp.Value.Single());

                newDefiniteIngredientsByAllergen.ToList().ForEach(x => {
                    definiteIngredientsByAllergen.Add(x.Key, x.Value);
                    possibleIngredientsByAllergen = possibleIngredientsByAllergen.ToDictionary(
                        kvp => kvp.Key,
                        kvp => kvp.Value.Except(new List<string>() {x.Value}).ToList()
                    );
                });
            }

            var safeIngredients = allIngredients.Except(definiteIngredientsByAllergen.Select(kvp => kvp.Value)).ToList();
            
            return safeIngredients.Sum(i => lines.Where(l => l.Item1.Contains(i)).Count());
        }
...
jgbk gvdstsc spnd rtmfg psqc njdb drtdz thtlt dxrsgs cldgd hjvzcp nggbtk dfcgd nppxr mbmtz gcpks rdjdq lzvh czvphx rjc vnbfvkp mhrlx frxmq bfnnnrn kmlkx qpbl lsgqf ldrgj qrpzt jp bvpxc phc zmsdzh jbtlfv nhzthvn rsr jxvx th rzcps kdbxxzv xgpdnz nqvnn rdkrtr pmhhqrk pdt prbk rbjmdn fgptl gpjr zbxj czmml tqkfx xdcp njqrhcc cncpbssj chgjqc fgcd (contains nuts, dairy, eggs)";
        private const string ProblemTestInput = @"mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
